    ...
    <?php foreach ($users as $user): ?>
        <div class="bg-white rounded-2xl border border-slate-100 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.1)] transition-all duration-300 hover:-translate-y-1 relative overflow-hidden group">
            ...
            <div class="p-6 relative z-10">
                ...
                <h3 class="text-lg font-bold text-slate-800 mb-1"><?= $user['username'] ?></h3>
                ...
                <div class="mt-6 pt-4 border-t border-slate-100 flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <a href="/admin/user/edit/<?= $user['id'] ?>" class="p-2 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Edit">
                        <i class="fas fa-edit"></i>
                    </a>
                    <a href="/admin/user/delete/<?= $user['id'] ?>" onclick="return confirm('Hapus user ini?')" class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Hapus">
